...
对窗口内每个需要的时间戳,收集各源该时刻的收盘价(四舍五入到 2 位小数)。**容差 `ε = max(0.05 元, 0.5% × 均价)`(相对容差)** —— 实测(2026-07-24 回填)发现盘中两源 15 分钟 bar 收盘价常差几分钱(不同 tick 聚合),原定 0.01 元绝对容差会把 ~60% 大盘股误判为 conflict;相对容差随价位缩放,只拦"整只错/量级错",放行 penny 噪声(信号是 MA 均线,几分钱单根噪声在均值中本就抹平):
